...
{
    public partial class Form_TTS_Single : Form
    {
        /// <summary>
        /// 接口url
        /// </summary>
        ...
        public Form_TTS_Single() { InitializeComponent(); }

        private void Form_TTSLauncher_Load(object sender, EventArgs e) { InitOpertion(); }
        ...
        {
            if(string.IsNullOrEmpty(textBox_AudioContent.Text) || string.IsNullOrEmpty(textBox_AudioName.Text))
            {
                MessageBoxButtons messButton = MessageBoxButtons.OK;
                MessageBoxEx.Show(this, "语音内容或名称不能为空", "警告", messButton);
                return;
            }
            ...
            SendJsonInfo sendJsonInfo = new SendJsonInfo()
            {
                ...
                text = textBox_AudioContent.Text,
                token = GetToken(),
                format = "wav",
            ...
            HttpPost(url, JsonMapper.ToJson(sendJsonInfo), dataCallBack: (data) => SaveAudioData(data, voiceOuputPath + @"\" + $"/{textBox_AudioName.Text}.wav"));
        }
